...
type ConfluenceConnectionDto = components['schemas']['ConfluenceConnectionDto']
...
function StatusBadge({ status }: { status: ConfluenceConnectionDto['lastTestStatus'] }) {
  if (status === 'Success')
    return (
...
export function ConfluenceSettings() {
  ...
  const { data: connection, isLoading } = useQuery<ConfluenceConnectionDto | null>({
    queryKey: ['confluence-connection'],
    queryFn: () =>
      apiFetch('/api/v1/integrations/confluence').then((r) =>
  ...
  const save = useMutation({
    mutationFn: async (data: FormData) => {
      const res = await apiFetch('/api/v1/integrations/confluence', {
        method: 'PUT',
        body: JSON.stringify(data),
      })
      if (!res.ok) throw new Error('Failed to save Confluence connection')
      return res.json()
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['confluence-connection'] }),
  })
  ...
  const handleTest = async () => {
    ...
    setTestMessage('')
    try {
      const res = await apiFetch('/api/v1/integrations/confluence/test', {
        method: 'POST',
        body: JSON.stringify(data),
      })
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Confluence</h2>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Connect to Confluence Cloud to publish release notes.
        </p>
      </div>
