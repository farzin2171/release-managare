...
**Feature Branch**: `007-project-page-templates`
**Created**: 2026-05-24
**Status**: Draft
...
- What happens when a multi-page publish partially succeeds (e.g. page 1 created, page 2 fails)? The system aborts on the first Confluence error, displays a per-page status list showing which pages succeeded and which failed, and allows the admin to re-run publish. Re-running is safe because publish is idempotent (FR-014): succeeded pages are updated in place rather than duplicated, so no cleanup or rollback is needed.
...
- **FR-003**: System MUST support per-project custom string key/value variables, surfaced to templates under the `{{custom.<key>}}` namespace. Custom variable keys MUST match `[a-zA-Z0-9_]` and MUST NOT exceed 64 characters; the system MUST reject keys that violate this format at creation/update time.
...
- **FR-006**: System MUST render Handlebars templates server-side with access to `project`, `version`, `previousVersion`, `releaseDate`, `repositories`, `tickets` (bucketed Breaking/Features/Fixes/Other), `contributors`, `reconciliation` (nullable), `confluence`, and `custom` variables. The Handlebars engine MUST be configured to expose only these declared context properties and the registered helpers (FR-007); templates MUST NOT have access to .NET runtime internals or any property outside `ReleaseRenderContext`.
...
- The wizard session is bounded to one user and one browser tab; no concurrent multi-user editing of prepared pages is supported in this feature. Session state (prepared page titles, bodies, dirty-edit flags) is held exclusively in browser React state and is not persisted server-side; closing or refreshing the tab discards in-progress edits.
...
- Q: What should the system do when a multi-page publish partially succeeds (e.g. page 1 created, page 2 fails)? → A: Abort on first failure; surface per-page status; admin re-runs to recover (idempotent re-run via FR-014 handles recovery without rollback).
- Q: Where should wizard session state (prepared page titles, bodies, dirty-edit flags) be stored? → A: Browser-only React state — lost on page refresh; no server-side draft record persisted.
- Q: Should admin-authored Handlebars templates be sandboxed or fully trusted? → A: Restrict to declared context only — templates may only access registered helpers and properties explicitly present in `ReleaseRenderContext`; no access to .NET internals.
- Q: What format constraints apply to custom variable keys? → A: Alphanumeric + underscore only (`[a-zA-Z0-9_]`), max 64 characters.
- Q: Can the same template be bound to a project more than once (e.g. under different kinds)? → A: Yes — a template may appear in multiple bindings for the same project; uniqueness is not enforced on (project + template ID).
